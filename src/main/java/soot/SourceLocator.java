...
import soot.util.ResourceCache;
...
public class SourceLocator {
  ...
  final ResourceCache<String, ZipFile> archivePathToZip = new ResourceCache<>(ZipFile::new);
      ...
      = CacheBuilder.newBuilder().initialCapacity(5).maximumSize(PATH_CACHE_CAPACITY).softValues()
          .concurrencyLevel(Runtime.getRuntime().availableProcessors()).build(new CacheLoader<String, Set<String>>() {
            ...
            public Set<String> load(String archivePath) throws Exception {
              try (SharedResource<ZipFile> archive = archivePathToZip.get(archivePath)) {
  ...
  public List<String> getClassesUnder(String aPath, String prefix) {
    ...
    if (cst == ClassSourceType.apk || cst == ClassSourceType.dex) {
      ...
      try (SharedResource<ZipFile> archive = archivePathToZip.get(aPath)) {
